...
fn main() -> Result<(), impl std::error::Error> {
    ...
    let redraw_closure = |ch: &mut Children, pc: &mut PhysicsContext, event: &WindowEvent| { 
        match event {
    ...
    let mut app = EngineContext::new(&event_loop, |event, ch, ph| window(event, ch, ph), |ch, pc, event| redraw_closure(ch, pc, event));
